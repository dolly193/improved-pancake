...
const getProducts = async () => {
    const { rows } = await pool.query('SELECT * FROM products WHERE is_active = true AND (stock > 0 OR stock = -1)');
...
const addProduct = async (product) => {
    const query = `
        INSERT INTO products (id, name, price, description, emoji, stock, is_active)
...
const deleteProduct = async (productId) => {
    // Soft delete: em vez de deletar, marcamos como inativo.
    const query = `
        UPDATE products
        SET is_active = false
        WHERE id = $1;
