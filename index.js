    ...
    if (interaction.isChatInputCommand()) { // Lida com comandos de barra
        ...
        if (commandName === 'comprar') {
            ...
            await interaction.editReply({
                content: `Você tem certeza que deseja **arquivar** o item **${product.name}** (ID: \`${productId}\`)? Ele não aparecerá mais para compra, mas o histórico de pedidos será mantido.`,
                components: [row],
            });
        ...
        if (action === 'confirm-delete') {
            ...
            if (product) {
                ...
                await logAuditEvent(`ADMIN: ${interaction.user.tag} (ID: ${interaction.user.id}) arquivou o produto "${productName}" (ID: ${id}).`);
                await interaction.update({ content: `✅ O produto **${productName}** foi arquivado com sucesso e não está mais à venda.`, components: [] });
